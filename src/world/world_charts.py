...
def chart(data, data_rolling, countries, by_million_inh = False, align_curves = False, last_d = 15,          offset_name = 'offset_confirmed', type_ppl = "confirmed cases", name_fig = "", since = False,           min_rate=0, log=False, new=""):
    ...
    fig.update_layout(
        ...
        annotations = [dict(xref='paper',
            ...
            x=0, y=1.05,
            showarrow=False,
            text ='Last update: {} ; Last data: {} ; Data: CSSE ; Author: @DJIBY CASSE & ALPHA SOW'.format(today, str(data['date'].values[-1])[:10]))]
    )
...
for (data, name_var, same_scale) in [(data_deaths_t, "deaths", True), (data_deaths_t, "deaths", False), (data_confirmed_t, "confirmed", True), (data_confirmed_t, "confirmed", False)]: 
    ...
    sub = "<sub>par ordre décroissant du cumul total, les croix représentent les données quotidiennes brutes et les bâtons la moyenne mobile sur 14 j.  •  berryllium</sub>"
...
for (dataf, name_fig, title) in [(data_deaths_t, "deaths_world", 'deaths'), (data_confirmed_t, "cases_world", 'cases')]:
    ...
    fig["layout"]["annotations"] += ( dict(
                            ...
                            xanchor='center',
                            yanchor='middle',
                            text='berryllium - {}'.format(datetime.strptime(max(data.index), '%Y-%m-%d').strftime('%d %B %Y')),
                            showarrow = False,
                            font=dict(size=15), 
                            opacity=0
                        ),
                            ...
                            yref='paper',
                            xanchor='center',
                            text='colored bars are a rolling mean of 7 days, grey x are raw data - berryllium',
                            font=dict(size=15),
                            showarrow = False),)

    fig.add_layout_image(
            dict(
                source="https://raw.githubusercontent.com/djibykc/berryllium-covid/master/images/autres/berryllium_logo_text.jpeg",
